...
def project_3DPoints(voxel, H):  #H is the transformation matrix
    # format X,Y,Z coordinates
    voxel = np.concatenate((np.array(voxel), np.ones((1, 128))), axis=0)
    ...
    new2D_coord = np.dot(H, np.transpose(voxel))
    scale = np.array([np.transpose(new2D_coord[2])])
    new2D_coord_final = np.divide(new2D_coord[:2, :], scale)
    ratio_x = 384.0 / np.float(xlim_im)
    ratio_y = 384.0 / np.float(ylim_im)
